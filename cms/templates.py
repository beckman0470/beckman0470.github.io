...
.breadcrumb{padding:28px 0 0;color:var(--muted);font-size:14px}.breadcrumb a{color:var(--brown);font-weight:800}.article-hero{padding:54px 0 38px}.article-hero h1{font-family:var(--serif);font-size:clamp(42px,6vw,76px);line-height:1.08;letter-spacing:-.04em;margin:16px 0 20px}.article-lead{font-size:22px;color:#344238;max-width:820px}.meta{display:flex;gap:12px;flex-wrap:wrap;margin-top:24px}.pill{display:inline-flex;padding:7px 12px;border-radius:999px;border:1px solid var(--line);background:var(--surface);font-size:14px;color:var(--muted);font-weight:800}.hero-cover{min-height:380px;border-radius:32px;background:linear-gradient(135deg,#C7D9D0,#FFF7EC);box-shadow:var(--shadow);border:1px solid var(--line);margin-top:34px;padding:42px}.hero-cover h2{font-family:var(--serif);font-size:58px;line-height:1.05;margin:0}.quote{margin:46px auto;background:var(--surface);border-left:6px solid var(--brown);border-radius:24px;padding:28px 32px;box-shadow:var(--shadow);font-size:25px;font-family:var(--serif);line-height:1.55}.article-body{font-size:19px}.article-body p{margin:0 0 1.4em}.article-body h2{font-size:32px;line-height:1.25;margin:2.2em 0 .7em}.article-body h3{font-size:25px;margin:1.8em 0 .5em}.article-body blockquote{background:#FFFDF8;border-left:5px solid var(--brown);padding:18px 22px;border-radius:18px;color:#344238}.btn-row{display:flex;gap:14px;flex-wrap:wrap;margin-top:34px}.btn{display:inline-flex;align-items:center;justify-content:center;padding:13px 24px;border-radius:999px;background:var(--ink);color:white;font-weight:950;box-shadow:0 12px 30px rgba(38,51,40,.16)}.btn.secondary{background:var(--surface);color:var(--ink);border:1px solid var(--line);box-shadow:none}
.article-nav{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:48px 0}.nav-card{background:var(--surface);border:1px solid var(--line);border-radius:24px;padding:20px;box-shadow:var(--shadow)}.nav-card strong{display:block;font-size:18px;line-height:1.35;margin-top:6px}.related{margin:58px 0}.related-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:16px}.related-card{background:var(--surface);border:1px solid var(--line);border-radius:24px;padding:20px;box-shadow:var(--shadow);transition:.2s ease}.related-card:hover{transform:translateY(-3px)}.related-card h3{font-size:20px;line-height:1.35;margin:8px 0}.chip{display:inline-flex;padding:5px 11px;border-radius:999px;background:#EEE6D9;color:var(--brown);font-size:13px;font-weight:950}
...
def render_prev_next(prev_next):
    previous = prev_next.get("previous") if prev_next else None
    next_item = prev_next.get("next") if prev_next else None

    if previous:
        left = f"""<a class="nav-card" href="{rel_url(previous.get('url'))}">
          <div class="kicker">上一篇</div>
          <strong>{previous.get('title')}</strong>
          <p class="muted">{previous.get('date','')}</p>
        </a>"""
    else:
        left = """<div class="nav-card"><div class="kicker">上一篇</div><strong>已經是最新一篇</strong></div>"""

    if next_item:
        right = f"""<a class="nav-card" href="{rel_url(next_item.get('url'))}">
          <div class="kicker">下一篇</div>
          <strong>{next_item.get('title')}</strong>
          <p class="muted">{next_item.get('date','')}</p>
        </a>"""
    else:
        right = """<div class="nav-card"><div class="kicker">下一篇</div><strong>已經是最後一篇</strong></div>"""

    return f'<div class="article-nav">{left}{right}</div>'

def render_related(related_items):
    if not related_items:
        return ""
    cards = []
    for item in related_items:
        cards.append(f"""<a class="related-card" href="{rel_url(item.get('url'))}">
          <span class="chip">{item.get('series') or item.get('category') or '故事'}</span>
          <h3>{item.get('title')}</h3>
          <p class="muted">{item.get('summary') or ''}</p>
        </a>""")
...
def render_article_page(meta: dict, html_body: str, related_items=None, prev_next=None):
    ...
    related_html = render_related(related_items or [])
    prev_next_html = render_prev_next(prev_next or {})
...
<header class="site-header">
  <div class="container nav">
    ...
    <nav class="nav-links"><a href="../index.html">首頁</a><a href="../articles.html">故事</a><a href="../series.html">系列</a><a href="../search.html">搜尋</a><a href="../tags.html">標籤</a><a href="../subscribe.html">訂閱</a><a href="../family.html">我們一家</a><a href="../about.html">關於</a></nav>
  </div>
</header>
...
<footer class="site-footer">
  <div class="container footer-grid">
    ...
    <div><h4>閱讀</h4><p><a href="../index.html">首頁</a></p><p><a href="../articles.html">故事</a></p><p><a href="../series.html">系列</a></p><p><a href="../search.html">搜尋</a></p><p><a href="../tags.html">標籤</a></p></div>
    <div><h4>訂閱</h4><p><a href="../subscribe.html">訂閱說明</a></p><p><a href="../rss.xml">RSS</a></p><p><a href="../sitemap.xml">Sitemap</a></p></div>
    <div><h4>品牌</h4><p><a href="../family.html">我們一家</a></p><p><a href="../about.html">關於</a></p><p><a href="https://vocus.cc/salon/jisoopama-baby-cat" target="_blank">Vocus</a></p></div>
  </div>
